...
export default function AdminUserSpaces() {
  ...
  useEffect(() => {
    if (userId) {
      setLoading(true);
      getSpacesByUserId(Number(userId), page, pageSize).then((res) => {
  ...
  // Copia el renderItem de MySpacesComponent para la paginación custom
  const renderItem = ({
    ref,
    key,
    value,
    ...
    <div className="mt-4">
      {spaces.length === 0 ? (
        <p className="flex justify-center text-2xl font-bold mb-4">
          No hay espacios para este usuario.
        </p>
      ) : (
        <>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            ...
            <Button
              ...
              onPress={async () => {
                if (spaceToDelete && userId) {
                  setLoading(true);
                  try {
                    await deleteSpace(Number(spaceToDelete.id), Number(userId));
                    setSpaceToDelete(null);
                    // Recargar espacios
                    getSpacesByUserId(Number(userId), page, pageSize).then(
                      (res) => {
                        setSpaces(res.spaces || res);
                        setTotal(
                          res.total || (res.spaces ? res.spaces.length : 0)
                        );
                        setLoading(false);
                      }
                    );
